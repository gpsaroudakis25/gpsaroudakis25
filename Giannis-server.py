...
def handle_client(client_socket, address):
    ...
    # Έλεγχος για duplicate usernames:
    if any(existing_username == username for sock, (addr, existing_username) in clients.items()):
        client_socket.send("Username already taken, goodbye.".encode())
        client_socket.close()
        return
    ...
    try:
        while True:
            ...
            if msg.lower() == "list":
                client_list = "\n".join([f"{user}" for sock, (addr, user) in clients.items()])
...
def main():
    ...
    Thread(target=auto_shutdown_timer, args=(shutdown_time_seconds,), daemon=True).start()
